...
class Questionnaire:
    ...
    def navigate_to_question(self, question_index: int) -> None:
        ...
        if 0 <= question_index < len(self.questions):
            self._set_current_step(question_index)

    def navigate_to_question_by_key(self, question_key: str) -> None:
        """Navigate directly to a specific question by its key."""
        for i, question in enumerate(self.questions):
            if question.key == question_key:
                self._set_current_step(i)
                break

    def get_completion_summary(self) -> Dict[str, Any]:
        """Get a summary of questionnaire completion status."""
        stored_data = self._get_stored_data()
        current_step = self._get_current_step()

        return {
            "total_questions": len(self.questions),
            "answered_questions": len(stored_data),
            "current_question": (
                current_step + 1
    ...
    def show_summary(self) -> None:
        """Display a summary view of all questions and answers."""
        st.title(f"📊 Overzicht: {self.name}")
        ...
        with col2:
            st.metric("Voortgang", f"{summary['completion_percentage']:.0f}%")
        with col3:
            st.metric("Huidige vraag", summary["current_question"])
        ...
        for i, question in enumerate(self.questions):
            with st.expander(
                f"Vraag {i + 1}: {question.text}",
                expanded=(i == self._get_current_step()),
            ):
                if question.key in stored_data:
                    st.success(f"✅ Beantwoord: {stored_data[question.key]}")
                else:
                    st.warning("⏳ Nog niet beantwoord")

                if question.help_text:
                    st.caption(f"💡 {question.help_text}")

                if st.button(f"Ga naar vraag {i + 1}", key=f"nav_summary_{i}"):
                    self.navigate_to_question(i)
                    st.rerun()
        ...
        with col2:
            if not self.is_complete():
                if st.button("▶️ Ga verder", type="primary", use_container_width=True):
                    ...
                    for i, question in enumerate(self.questions):
                        if question.key not in stored_data:
                            self.navigate_to_question(i)
                            st.rerun()
                            break
    ...
    def run(self) -> Optional[Dict[str, Any]]:
        """Run the questionnaire with enhanced navigation.
        ...
        if current_step >= len(self.questions):
            ...
            data = self.get_data()
            if data and self.config.show_previous_answers:
                with st.expander("📋 Overzicht van alle antwoorden", expanded=False):
                    for i, question in enumerate(self.questions):
                        value = data.get(question.key, "Niet beantwoord")
                        st.write(f"**{i + 1}. {question.text}:** {value}")
        ...
        # Render current question with enhanced styling
        ...
        # Enhanced navigation with jump-to buttons
        self._render_enhanced_navigation(current_step, current_value)
    ...
    def _render_enhanced_navigation(
        ...
        """Render enhanced navigation with jump-to capabilities."""
        total_steps = len(self.questions)
        question = self.questions[current_step]
        ...
        with nav_cols[0]:
            if current_step > 0:
                if st.button(
                    "⏮️ Eerste",
                    key=f"first_{current_step}",
                    help="Ga naar de eerste vraag",
                ):
                    self._store_answer(question.key, current_value)
        ...
        with nav_cols[2]:
            if current_step < total_steps - 1:
                button_text = "Volgende ➡️"
                button_key = f"next_{current_step}"
                button_type = "primary"
            else:
                button_text = "Voltooien ✅"
                button_key = f"complete_{current_step}"
                button_type = "primary"
        ...
        with nav_cols[3]:
            if current_step < total_steps - 1:
                if st.button(
                    "⏭️ Laatste",
                    key=f"last_{current_step}",
                    help="Ga naar de laatste vraag",
                ):
                    self._store_answer(question.key, current_value)
                    self._set_current_step(total_steps - 1)
                    st.rerun()

        # Add quick jump section for longer questionnaires
        if total_steps > 5:
            st.write("---")
            st.caption("🎯 Snelle navigatie:")

            # Max 8 buttons per row
            jump_cols = st.columns(min(total_steps, 8))
            for i in range(min(total_steps, 8)):
                with jump_cols[i]:
                    if i != current_step:
                        if st.button(
                            f"{i + 1}",
                            key=f"jump_{current_step}_{i}",
                            use_container_width=True,
                            help=f"Ga naar vraag {i + 1}",
                        ):
                            self._store_answer(question.key, current_value)
                            self._set_current_step(i)
                            st.rerun()
