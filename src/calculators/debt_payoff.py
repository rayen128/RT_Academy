    ...
    for i in range(int(num_debts)):
        ...
        with col2:
            balance = st.number_input(
                f'Openstaand bedrag {i + 1} (€)', min_value=0.0, value=1000.0 * (i + 1), step=100.0)
        with col3:
            rate = st.number_input(
                f'Rente {i + 1} (%)', min_value=0.0, max_value=100.0, value=5.0 + i, step=0.1)
        with col4:
            min_payment = st.number_input(f'Minimum betaling {i + 1} (€)',
                                          min_value=max(10.0, balance * 0.01),
                                          value=max(50.0, balance * 0.02),
                                          step=10.0)

        debts.append(Debt(name=name, balance=balance,
                     interest_rate=rate, min_payment=min_payment))
...
def create_payoff_graph(snowball_data: Dict[str, List[float]],
    ...
    first_snowball_values = list(snowball_data.values())[
        0] if snowball_data else []
    ...
    for month in range(len(first_snowball_values)):
        try:
            snowball_total = sum(balances[month]
                                 for balances in snowball_data.values())
            avalanche_total = sum(balances[month]
                                  for balances in avalanche_data.values())
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color='#2ecc71', width=2),  # Groen
        fill='tozeroy'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color='#3498db', width=2)  # Blauw
    ))

    # Calculate initial total debt for y-axis range
    initial_debt = max(snowball_totals[0], avalanche_totals[0])
...
def display_results(snowball_results: Tuple[Dict[str, List[float]], float, int],
    ...
    snowball_balances, snowball_total, snowball_months = snowball_results
    avalanche_balances, avalanche_total, avalanche_months = avalanche_results
    ...
    if avalanche_total < snowball_total:
        st.success(
            f'De Avalanche methode bespaart je €{savings:,.2f} en is {faster_months} maanden sneller!')
    elif snowball_total < avalanche_total:
        st.success(
            f'De Snowball methode bespaart je €{savings:,.2f} en is {faster_months} maanden sneller!')
    else:
...
def show_debt_payoff_calculator():
    ...
    with st.expander("💳 Schuld Aflossing Calculator (Snowball vs. Avalanche)", expanded=False):
        st.write("""
        **Snowball Methode**: Betaal de kleinste schuld eerst af (motiverender door snelle overwinningen)
        
        **Avalanche Methode**: Betaal de schuld met hoogste rente eerst af (wiskundig optimaal)
        """)
        ...
        if calculate_clicked and params.debts:  # Check if we have any debts
            try:
                ...
                if snowball_results[0] and avalanche_results[0]:
                    ...
                    if snowball_balances and avalanche_balances and len(list(snowball_balances.values())[0]) > 0:
                        ...
                        st.warning(
                            "Niet genoeg gegevens om een grafiek te maken. Controleer je invoer.")
            except Exception as e:
                st.error(
                    f"Er is een fout opgetreden bij het berekenen. Controleer je invoer en probeer opnieuw.")
